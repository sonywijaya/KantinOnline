...
$grabdata = mysql_query("select name, password, phonenumber from seller  where email = '$email'");
...
if(isset($_POST['submit'])){
    if (empty($_POST['name']) || empty($_POST['phone']) || empty($_POST['password'])) {
        ...
        $phone = $_POST['phone'];
        if (mysql_query("update seller set name = '$name', phonenumber = '$phone',password = '$password' where email = '$email'")) {
            echo "<script language='javascript' type='text/javascript'>
               alert('Profile Updated!');
               location.href='sellerprofile.php'
               </script>";
        } else {
            echo "<script language='javascript' type='text/javascript'>
...
<div class="main-nav">
    <ul class="nav">
        <li class="pageName">Seller Profile</li>
