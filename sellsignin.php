...
if(isset($_POST['login'])){
    ...
    $query=mysql_query("select email, password from seller where email = '$email' AND  password = '$password'");
...
if(isset($_POST['register'])){
    ...
    if (empty($_POST['email']) || empty($_POST['password']) || empty($_POST['name']) || empty($_POST['phone'])) {
        ...
        if(mysql_query("insert into seller values('$name', '$email', '$password', '$phone')")) {
            echo "<script language='javascript' type='text/javascript'>";
...
<head>
    <meta charset="UTF-8">
    <title>Seller Login | Kantin Online</title>
...
<div class="main-nav">
    <ul class="nav">
        <li class="pageName">Seller Login</li>
    ...
    <div class="login-page">
        <div class="form">
            <form class="register-form" action="" method="post">
                <center><h2>Seller Registration</h2></center>
                <?php
                if (isset($_SESSION['buyer'])) {
                    echo '
                    <input type="text" name="name" placeholder="Please Sign Out From Buyer Account" disabled>
                    <input type="password" name="password" placeholder="Please Sign Out From Buyer Account" disabled>
                    <input type="email" name="email" placeholder="Please Sign Out From Buyer Account." disabled>
                    <input type="tel" name="phone" placeholder="Please Sign Out From Buyer Account" disabled>
                    <input type="submit" name="register" value="Please Sign Out From Buyer Account" class="button" style="background: #b3b3b3" disabled>
                    ';
                }
                else echo '
                <input type="text" name="name" placeholder="store name" maxlength="30">
                ...
                <input type="submit" name="register" value="Create" class="button" style="background: #0078e7">
                ';
                ?>
                <p class="message">Already registered? <a href="#" style="color: #0078e7">Sign In</a></p>
            </form>
            <form class="login-form" action="" method="post">
                <center><h2>Seller Login</h2></center>
                <?php
                if (isset($_SESSION['buyer'])) {
                    echo '
                    <input type="email" name="email" placeholder="Please Sign Out From Buyer Account" disabled>
                    <input type="password" name="password" placeholder="Please Sign Out From Buyer Account"disabled>
                    <input type="submit" name="register" value="Please Sign Out From Buyer Acc." class="button" style="background: #b3b3b3" disabled>
                    ';
                }
                else echo '
                <input type="email" name="email" placeholder="email address" maxlength="30">
                <input type="password" name="password" placeholder="password" maxlength="30">
                <input type="submit" name="login" value="Login" class="button" style="background: #0078e7">
                ';
                ?>
                <p class="message">Not registered? <a href="#" style="color: #0078e7">Create an account</a></p>
            </form>
        </div>
    </div>
